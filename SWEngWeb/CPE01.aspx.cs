        ...
        protected void queryToDB()
        {
            using (SqlConnection connection = new SqlConnection(WebConfigurationManager.ConnectionStrings["DBConnection"].ConnectionString))
            {
                ...
                if (adviser.SelectedIndex != 0)
                {
                    ...
                    cmd.Parameters.AddWithValue("@requestDateTime", myDateTime.ToString("d MMM yyyy HH:mm:ss"));
                    connection.Open();
                    cmd.ExecuteNonQuery();
                    connection.Close();
                }
                ...
                cmd.Parameters.AddWithValue("@dateTime", myDateTime.ToString("d MMM yyyy HH:mm:ss"));
                connection.Open();
                cmd.ExecuteNonQuery();
                connection.Close();
